...
class CalculationHistoryController extends Controller
{
    ...
    {
        $filename = 'calculations_' . now()->format('Y-m-d_H-i-s') . '.pdf';

        // Generate HTML content for PDF
        $html = $this->generatePdfHtml($calculations);
     ...
     */
    private function generatePdfHtml($calculations)
    {
        $html = '
        <!DOCTYPE html>
        <html>
        <head>
            <meta charset="utf-8">
            <title>Planting Calculations Export</title>
            <style>
        ...
        <body>
            <div class="export-date">Exported on: ' . now()->format('Y-m-d H:i:s') . '</div>
            <h1>Planting Calculations Export</h1>
            <div class="header-info">
                <strong>Total Calculations:</strong> ' . $calculations->count() . '<br>
                <strong>Generated by:</strong> ' . auth()->user()->name . ' (' . auth()->user()->email . ')
            </div>
            <table>
                <thead>
                    <tr>
        ...
        foreach ($calculations as $calc) {
            $html .= '
                    <tr>
                        <td>' . $calc->created_at->setTimezone(config('app.timezone'))->format('Y-m-d H:i:s') . '</td>
                        <td>' . ($calc->calculation_name ?? 'Unnamed') . '</td>
                        <td>' . ($calc->plant_type ?? 'Not specified') . '</td>
                        <td>' . $calc->area_length . '</td>
                        <td>' . $calc->area_width . '</td>
                        <td>' . $calc->plant_spacing . '</td>
                        <td>' . $calc->total_plants . '</td>
                        <td>' . $calc->total_area . '</td>
                        <td>' . ($calc->calculation_type ?? 'Square') . '</td>
                        <td>' . ($calc->notes ?? '') . '</td>
                    </tr>';
        }
